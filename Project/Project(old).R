...
colnames(Data) <- c("Status","FoundingYear","Age","FocusFunctions",
                    ...
                    "NumCofounders","NumAdvisors","SeniorLeadershipTeamSize", 
                    "TopCompanyExp","StartupExp","SucessfulStartupExp",
                    ...
                    "CrowdfundingBased","B2CorB2B","GlobalExporsure",
...
#Data$EmployeeCount <- as.integer(Data$EmployeeCount, na.rm = TRUE)
...
#Data[is.na(Data$EmployeeCount),"EmployeeCount"]<-mean(Data$EmployeeCount,na.rm=TRUE) 
...
# Replace NA's of Yes/No Factors Columns with Mode
Mode <- function(x) {
  ux <- unique(x)
